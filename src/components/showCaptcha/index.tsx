...
export default function ShowCaptcha() {
    ...
    const fetchCaptchas = async () => {
        try {
            const response = await fetch('/api/v1/demo/captchas');
    ...
    const solveCaptcha = async () => {
        try {
            const response = await fetch('/api/v1/demo/solve', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="flex flex-col items-center justify-center">
            <div className={`flex flex-col h-26 items-center justify-center ${currentCaptcha.includes("dark") ? "bg-[#555555] border-gray-800" : "bg-[#FAFAFA] border-gray-400"} rounded-md border-2 `}>
                <h3 className={`font-bold ${currentCaptcha.includes("dark") ? "text-white" : "text-black"}`}>IconCaptcha Challenge</h3>
                {!currentCaptcha && (
                    <div role="status">
                ...
                <h3 className={`font-bold text-[12px] ${currentCaptcha.includes("dark") ? "text-white" : "text-cyan-600"}`}>
                    {!icon ? (
                        <>IconCaptcha &copy;</>
                    ) : (
