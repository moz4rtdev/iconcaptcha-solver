...
const solve = async () => {
  const response = await axios.post('https://iconcaptcha-solver.vercel.app/api/v2/solve', {
    token: 'YOUR_TOKEN',
    image: 'BASE64_IMG'
  });
...
response = requests.post(
    'https://iconcaptcha-solver.vercel.app/api/v2/solve',
    json={
        'token': 'YOUR_TOKEN',
        'image': 'BASE64_IMG'
    }
...
$ch = curl_init('https://iconcaptcha-solver.vercel.app/api/v2/solve');
...
public class IconCaptchaSolver {
    public static void main(String[] args) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://iconcaptcha-solver.vercel.app/api/v2/solve"))
  ...
  curl: {
    ...
    code: `curl -X POST https://iconcaptcha-solver.vercel.app/api/v2/solve \\
    ...
    <div className="w-full">
      ...
      <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10 shadow-xl hover:border-white/20 transition-colors">
        <pre className="bg-[#2d2d2d] p-4 rounded-lg overflow-x-auto">
          <code
            className={`language-${activeTab === "curl" ? "bash" : activeTab === "nodejs" ? "javascript" : activeTab}`}
          >
